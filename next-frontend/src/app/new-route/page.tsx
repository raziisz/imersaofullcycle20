...
export async function searchDirection(source: string, destination: string) {
  console.log(process.env.NEST_API_URL, 'teste')
  ...
  if (!sourceResponse.ok) {
    console.error(await sourceResponse.text(), await sourceResponse);
...
export async function NewRoutePage({
  ...
  const result =
    source && destination ? await searchDirection(source, destination) : null;
        ...
        {directionsData && (
          <div className="mt-4 p-4 border rounded text-contrast">
            ...
            <NewRouteForm>
              ...
              <button type="submit" className="bg-main text-primary font-bold p-2 rounded mt-4">Adicionar rota</button>
            </NewRouteForm>
          </div>
        )}
      </div>
